...
public class GestionContabilidad implements IGestionContabilidad {
    ...
    public GestionContabilidad(IGestionMovimientoContable gestionMovimientoContable) {
        ...
        try {
            movimientos = gestionMovimientoContable.listarMovimientos();
        } catch (Exception e) {
    ...
    public void registrarIngresoPorVenta(Connection conexion, Venta venta) throws Exception {
        ...
        movimientosAGuardar.add(crearMovimiento(movimientosAGuardar, fecha, TipoMovimientoContable.INGRESO,
                cuentaIngreso, venta.getTotal(), "Ingreso por venta " + venta.getNumeroFactura(),
                ORIGEN_VENTA, venta.getNumeroFactura()));

        movimientosAGuardar.add(crearMovimiento(movimientosAGuardar, fecha, TipoMovimientoContable.INGRESO,
                CUENTA_INGRESOS, venta.getSubTotal(), "Venta " + venta.getNumeroFactura(),
                ORIGEN_VENTA, venta.getNumeroFactura()));

        if (venta.getImpuestos() > 0) {
            movimientosAGuardar.add(crearMovimiento(movimientosAGuardar, fecha, TipoMovimientoContable.INGRESO,
                    CUENTA_IVA_GENERADO, venta.getImpuestos(), "IVA generado venta " + venta.getNumeroFactura(),
                    ORIGEN_VENTA, venta.getNumeroFactura()));
        }
    ...
    public void registrarEgresoPorCompra(Connection conexion, Compra compra) throws Exception {
        ...
        movimientosAGuardar.add(crearMovimiento(movimientosAGuardar, fecha, TipoMovimientoContable.EGRESO,
                CUENTA_INVENTARIO, compra.getSubtotal(), "Compra " + compra.getNumeroFacturaProveedor(),
                ORIGEN_COMPRA, compra.getNumeroFacturaProveedor()));
    ...
    public void registrarReversoPorAnulacionCompra(Connection conexion, Compra compra, String motivo) throws Exception {
        ...
        movimientosAGuardar.add(crearMovimiento(movimientosAGuardar, fecha, TipoMovimientoContable.INGRESO,
                cuentaPago, compra.getTotalCompra(), "Reverso egreso por anulacion compra " + referencia
                        + ". Motivo: " + motivo, ORIGEN_ANULACION_COMPRA, referencia));
        ...
        if (compra.getImpuestos() > 0) {
            movimientosAGuardar.add(crearMovimiento(movimientosAGuardar, fecha, TipoMovimientoContable.INGRESO,
                    CUENTA_IVA_DESCONTABLE, compra.getImpuestos(), "Reverso IVA compra " + referencia
                            + ". Motivo: " + motivo, ORIGEN_ANULACION_COMPRA, referencia));
        }
    ...
    public void registrarReversoPorAnulacionVenta(Connection conexion, Venta venta, String motivo) throws Exception {
        ...
        movimientosAGuardar.add(crearMovimiento(movimientosAGuardar, fecha, TipoMovimientoContable.EGRESO,
                cuentaIngreso, venta.getTotal(), "Reverso ingreso por anulacion venta " + referencia
                        + ". Motivo: " + motivo, ORIGEN_ANULACION_VENTA, referencia));
        ...
        if (venta.getImpuestos() > 0) {
            movimientosAGuardar.add(crearMovimiento(movimientosAGuardar, fecha, TipoMovimientoContable.EGRESO,
                    CUENTA_IVA_GENERADO, venta.getImpuestos(), "Reverso IVA generado venta " + referencia
                            + ". Motivo: " + motivo, ORIGEN_ANULACION_VENTA, referencia));
        }
    ...
    public void registrarReversoPorDevolucionVenta(Connection conexion, Venta venta, double subtotalDevuelto,
        ...
        movimientosAGuardar.add(crearMovimiento(movimientosAGuardar, fecha, TipoMovimientoContable.EGRESO,
                cuentaIngreso, totalDevuelto, "Reverso ingreso por devolucion venta " + referencia
                        + ". Motivo: " + motivo, ORIGEN_DEVOLUCION_VENTA, referencia));
        ...
        if (ivaDevuelto > 0) {
            movimientosAGuardar.add(crearMovimiento(movimientosAGuardar, fecha, TipoMovimientoContable.EGRESO,
                    CUENTA_IVA_GENERADO, ivaDevuelto, "Reverso IVA devolucion venta " + referencia
                            + ". Motivo: " + motivo, ORIGEN_DEVOLUCION_VENTA, referencia));
        }
    ...
    public MovimientoContable buscarMovimientoPorCodigo(String codigoTransaccion) {
        ...
        try {
            return gestionMovimientoContable.buscarPorCodigo(codigoTransaccion);
        } catch (Exception e) {
    ...
    private void guardarMovimientos(Connection conexion, List<MovimientoContable> movimientosAGuardar)
            throws Exception {
        gestionMovimientoContable.guardarMovimientos(conexion, movimientosAGuardar);
    ...
    private void validarVenta(Venta venta) throws Exception {
        ...
        if (venta.getFormaPago() == null) {
            throw new Exception("La venta no tiene forma de pago.");
        }
    }
