...
public class GestionCliente {
	...
	public GestionCliente(IGestionCliente gestionCliente) {
		if (gestionCliente == null) {
			throw new IllegalArgumentException("La gestionCliente no puede ser nula.");
		}

		this.gestionCliente = gestionCliente;

		try {
			clientes = gestionCliente.listar();
		} catch (Exception e) {
	...
	public Cliente buscarClientePorCodigo(String codigo) {
		try {
			return gestionCliente.buscar(codigo);
		} catch (Exception e) {
			throw new IllegalStateException("Error al buscar cliente por codigo: " + codigo, e);
		}
	}
	...
	public void actualizarCliente(Cliente clienteActualizado) throws Exception {
		...
		gestionCliente.actualizar(clienteActualizado);
	...
	public void cambiarEstadoCliente(String codigo) throws Exception {
		...
		gestionCliente.cambiarEstado(codigo);
	...
	private void validarNumeroIdentificacion(Cliente cliente) throws Exception {
		...
		case CC:
			if (!numero.matches("\\d{7,10}")) {
				throw new Exception("La Cédula de Ciudadanía debe contener entre 7 y 10 dígitos.");
			}
			break;
		...
		case CE:
			if (!numero.matches("\\d{6,12}")) {
				throw new Exception("La Cédula de Extranjería debe contener entre 6 y 12 dígitos.");
			}
			break;

		case PA:
			if (!numero.matches("[A-Za-z0-9]{5,20}")) {
				throw new Exception("El Pasaporte debe contener entre 5 y 20 caracteres alfanuméricos.");
			}
			break;
	...
	private void validarTelefono(String telefono) throws Exception {
		...
		if (!telefono.matches("^3\\d{9}$")) {
			throw new Exception("El teléfono debe iniciar por 3 y tener exactamente 10 dígitos.");
		}
	}
}