...
public class PanelProveedor extends JPanel {
	...
	private void construirPanel() {
		...
		// Se usan las constantes de Eventos para los textos
		...
		Campos.add(lRazonSocialProveedor);
		Campos.add(tRazonSocialProveedor);
		Campos.add(lNitProveedor);
		Campos.add(tNitProveedor);
		Campos.add(lDireccionProveedor);
		Campos.add(tDireccionProveedor);
		Campos.add(lTelefonoProveedor);
		Campos.add(tTelefonoProveedor);
		Campos.add(lCorreoProveedor);
		Campos.add(tCorreoProveedor);
	...
	// Puente para el ProveedorControlador
	public void setControlador(ActionListener controlador) {
		bRegistrar.addActionListener(controlador);
		bRegistrar.setActionCommand("REGISTRAR_PROVEEDOR");
		bModificar.addActionListener(controlador);
		bModificar.setActionCommand("MODIFICAR_PROVEEDOR");
		bInactivar.addActionListener(controlador);
		bInactivar.setActionCommand("INACTIVAR_PROVEEDOR");
		bBuscar.addActionListener(controlador);
		bBuscar.setActionCommand("BUSCAR_PROVEEDOR");
		bLimpiar.addActionListener(controlador);
		bLimpiar.setActionCommand("LIMPIAR_PROVEEDOR");
		bVolver.addActionListener(controlador);
		bVolver.setActionCommand("VOLVER_PROVEEDORES");
	}
	...
	public void poblarTabla(List<proveedorDto> lista) {
		...
		for (proveedorDto p : lista) {
			modeloTabla.addRow(new Object[] { p.getCodigoProveedor(), p.getRazonSocial(), p.getNit(), p.getDireccion(),
	...
	public proveedorDto getDatosProveedorModificar() {
		...
		proveedorDto proveedor = new proveedorDto();
		proveedor.setCodigoProveedor(codigoSeleccionado);
		proveedor.setRazonSocial(razonSocial);
		proveedor.setNit(tNitProveedor.getText());
		proveedor.setDireccion(tDireccionProveedor.getText());
		try {
