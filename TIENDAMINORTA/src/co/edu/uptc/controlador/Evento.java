...
public class Evento implements ActionListener {
    ...
    private void abrirModuloContabilidad() {
        ...
        contabilidadGUI.getBtnActualizar().addActionListener(this);  
        contabilidadGUI.getBtnEliminar().addActionListener(this);
    ...
    private void registrarCompra() {
        ...
        try {
            ...
            Compra compra = compraGUI.obtenerCompraFormulario();
            ...
            if (registrada) {
                compraGUI.mostrarMensaje("Compra registrada correctamente");

                compraGUI.cargarTabla(gestionCompra.listarCompras());
            ...
            compraGUI.mostrarMensaje("Error al registrar compra");
        }
    } 
    ...
    private void actualizarCompra() {
        ...
        // ✅ VALIDACIÓN CORRECTA → POR FACTURA
        if (compra == null || compra.getNumeroFacturaProveedor() == null
                || compra.getNumeroFacturaProveedor().trim().isEmpty()) {

            compraGUI.mostrarMensaje("Debe ingresar o seleccionar una factura válida");
            return;
        }

        System.out.println("FACTURA ENVIADA: " + compra.getNumeroFacturaProveedor());

        boolean modificado = gestionCompra.actualizarCompra(compra);

        if (modificado) {
            compraGUI.mostrarMensaje("Compra actualizada correctamente");
            compraGUI.cargarTabla(gestionCompra.listarCompras());
            compraGUI.limpiarFormulario();
        } else {
            compraGUI.mostrarMensaje("No se encontró la compra");
        }
    }
    ...
    private void eliminarCompra() {
        ...
        String codigo = compraGUI.getFacturaProveedor();

        if (codigo == null || codigo.trim().isEmpty()) {
            compraGUI.mostrarMensaje("Debe ingresar o seleccionar una factura");
            return;
        }

        boolean eliminado = gestionCompra.eliminarCompra(codigo);

        if (eliminado) {
            compraGUI.mostrarMensaje("Compra eliminada correctamente");
            compraGUI.limpiarFormulario();
        } else {
            compraGUI.mostrarMensaje("No se encontró la compra");
        }
    ...
    private void actualizarMovimiento() {
        ...
        MovimientoContable movimientoContable = contabilidadGUI.obtenerMovimientoFormulario();

        if (movimientoContable == null || movimientoContable.getCodigoTransaccion() == null) {
            contabilidadGUI.mostrarMensaje("Debe seleccionar un movimiento válido");
            return;
        }

        System.out.println("CODIGO ENVIADO: " + movimientoContable.getCodigoTransaccion());

        boolean modificado = gestionContabilidad.actualizarMovimiento(movimientoContable);

        if (modificado) {
            contabilidadGUI.mostrarMensaje("Movimiento contable actualizado correctamente");
            contabilidadGUI.cargarTabla(gestionContabilidad.listarMovimientos());
            contabilidadGUI.limpiarFormulario();
        } else {
            contabilidadGUI.mostrarMensaje("No se encontró el movimiento contable");
        }
    }

    private void eliminarMovimiento() {

        String codigo = contabilidadGUI.obtenerCodigoMovimiento();

        if (codigo == null || codigo.trim().isEmpty()) {
            contabilidadGUI.mostrarMensaje("Debe ingresar o seleccionar un código");
            return;
        }

        boolean eliminado = gestionContabilidad.eliminarMovimiento(codigo);

        if (eliminado) {
            contabilidadGUI.mostrarMensaje("Movimiento contable eliminado correctamente");
            contabilidadGUI.cargarTabla(gestionContabilidad.listarMovimientos());
            contabilidadGUI.limpiarFormulario();
        } else {
            contabilidadGUI.mostrarMensaje("No se encontró el movimiento contable");
