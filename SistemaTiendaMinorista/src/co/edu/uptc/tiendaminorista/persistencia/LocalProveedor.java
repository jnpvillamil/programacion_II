...
public class LocalProveedor implements IGestionProveedor {
    ...
    private List<Proveedor> leer() {
        try (FileReader reader = new FileReader(RUTA)) {
            Type tipo = new TypeToken<List<Proveedor>>() {}.getType();
            List<Proveedor> lista = gson.fromJson(reader, tipo);
    ...
    @Override
    public List<Proveedor> listar() { return leer(); }
    ...
    public void desactivar(String codigo) {
        ...
        for (Proveedor p : lista) {
            if (p.getCodigo().equals(codigo)) { p.setActivo(false); break; }
        }
        escribir(lista);
    }
    ...
    public void activar(String codigo) {
        List<Proveedor> lista = leer();
        for (Proveedor p : lista) {
            if (p.getCodigo().equals(codigo)) { p.setActivo(true); break; }
        }
        escribir(lista);
    }
}
