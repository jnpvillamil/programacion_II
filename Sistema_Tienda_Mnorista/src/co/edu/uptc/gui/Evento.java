...
public class Evento implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (evento.equals(SALIR)) {
            ...
            // Buscar cliente existente para preservar tipoDoc y tipoCliente
            co.edu.uptc.Datos.Clientedt existente = null;
            for (co.edu.uptc.Datos.Clientedt c : co.edu.uptc.negocio.Cliente_negocio.getInstance().getListaClientes()) {
                if (c.getNumerodocli().equals(doc)) {
                    existente = c;
                    break;
                }
            }
            ...
            if (existente != null) {
                co.edu.uptc.Datos.Clientedt cliente = new co.edu.uptc.Datos.Clientedt(
                        nombre,
                        existente.getTipodocli(),
                        doc,
                        direccion,
                        telefono,
                        existente.getTipocli()
                );
            ...
            Panel_Registrarpro panel = ventana.getPanelRegistroPro();

            String razon = panel.getRazon();
            String nit = panel.getNit();
            String direccion = panel.getDireccion();
            String telefono = panel.getTelefono();
            String correo = panel.getCorreo();

            if (razon.isEmpty() || nit.isEmpty() || direccion.isEmpty() || telefono.isEmpty() || correo.isEmpty()) {
                JOptionPane.showMessageDialog(null, "Todos los campos son obligatorios");
                return;
            }

            // Verificar duplicados
            for (co.edu.uptc.Datos.Proveedordt p : co.edu.uptc.negocio.Proveedor_negocio.getInstance().getListaProveedores()) {
            ...
            co.edu.uptc.Datos.Proveedordt proveedor = new co.edu.uptc.Datos.Proveedordt(razon, nit, direccion, telefono, correo);

            co.edu.uptc.negocio.Proveedor_negocio.getInstance().agregarProveedor(proveedor);

            ventana.getPanelProveedores().cargarProveedores(co.edu.uptc.negocio.Proveedor_negocio.getInstance().getListaProveedores());

            JOptionPane.showMessageDialog(null, "Proveedor registrado correctamente");

            ventana.mostrarProveedores();
            ...
            JOptionPane.showMessageDialog(null, "Actualización de proveedor no implementada aún");
            ventana.regresarcli();
        } else if (e.getActionCommand().equals(DESACTIVARCLI)) {

            String doc = ventana.getPanelActualizar().getClienteSeleccionado();

            co.edu.uptc.negocio.Cliente_negocio.getInstance().desactivarCliente(doc);
            ...
            co.edu.uptc.negocio.Cliente_negocio.getInstance().activarCliente(doc);
