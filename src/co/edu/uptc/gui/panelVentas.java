...
import co.edu.uptc.negocio.ventaDto;
...
public class panelVentas extends JPanel {
    ...
    public JTextField tNumeroFactura;
    ...
    private void construirPanel() {
        ...
        JPanel Campos = new JPanel(new GridLayout(8, 2, 8, 8));
        Campos.setBorder(BorderFactory.createTitledBorder("informacion de factura"));

        lNumeroFactura = new JLabel("NUMERO FACTURA:");
        ...
        tNumeroFactura = new JTextField(10);
        ...
        Campos.add(lNumeroFactura); 
        Campos.add(tNumeroFactura);
        Campos.add(lFechaHora);     
        Campos.add(tFechaHora);
        Campos.add(lCliente);       
        Campos.add(tCliente);
        Campos.add(lFormaPago);     
        Campos.add(cbFormaPago);
        Campos.add(lIva);           
        Campos.add(chkIva);
        Campos.add(lTotal);         
        Campos.add(tTotal);
        Campos.add(lRetencion);     
        Campos.add(tRetencion);
        Campos.add(lTotalFinal);    
        Campos.add(tTotalFinal);
        ...
        modeloProductos = new DefaultTableModel(columnas, 0) {
            ...
            public boolean isCellEditable(int row, int col) {
                return col != 4; // subtotal no editable
            }
        };
        ...
        bAgregarProducto.addActionListener(e ->modeloProductos.addRow(new Object[]{"", "", "1", "0.0", "0.0"})
        );
    ...
    public ventaDto getDatosVenta() {
        String numeroFactura = tNumeroFactura.getText();
        String cliente       = tCliente.getText();
        if (numeroFactura == null || numeroFactura.isBlank()) {
            JOptionPane.showMessageDialog(this, "El numero de factura es requerido");
            return null;
        }
    ...
    public String getNumeroFactura() {
        String numero = tNumeroFactura.getText();
